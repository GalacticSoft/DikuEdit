...
// ProgEditWnd syntax coloring class
//
...
//IMPLEMENT_CONTROL(ProgEditWnd, CWnd);
IMPLEMENT_DYNAMIC(ProgEditWnd, CWnd);

/////////////////////////////////////////////////////////////////////////////
// ProgEditWnd

BEGIN_MESSAGE_MAP(ProgEditWnd, CWnd)
//{{AFX_MSG_MAP(ProgEditWnd)
   ON_WM_ERASEBKGND()
   ON_WM_KEYDOWN()
   ON_WM_CHAR()
   ON_WM_LBUTTONDOWN()
...
/////////////////////////////////////////////////////////////////////////////
// ProgEditWnd construction/destruction

/////////////////////////////////////////////////////////////////////////////

ProgEditWnd::ProgEditWnd()
{
   // don't use this one
   ASSERT(0);

   m_className.Format("ProgEditWnd::%p", this);
...
ProgEditWnd::ProgEditWnd(CWnd * parent, 
                           const CRect & rect,
                           int nID, 
                           int iTabSize,
                           int iFontSize, 
                           const char *pFontName,
                           bool bHasHorizScroll)
{
   m_className.Format("ProgEditWnd::%p", this);
   //g_statusLog.StatusOut("%s, Constructing", m_className);
...
BOOL ProgEditWnd::Create(DWORD dwFlags,
                          CWnd * parent, 
                          const CRect & rect,
                          int nID, 
                          int iTabSize,
...
void ProgEditWnd::LoadText(const char *buffer)
{
...
void ProgEditWnd::UnloadText(CString &pText) const
{
...
ProgEditWnd::~ProgEditWnd()
{
   //g_statusLog.StatusOut("%s, ~ProgEditWnd", m_className);
...
CPoint ProgEditWnd::GetDeviceScrollPosition() const
{
...
CPoint ProgEditWnd::GetScrollPosition() const   // logical coordinates
{
...
int ProgEditWnd::VisibleColumns() const
{
   if (!::IsWindow(m_hWnd))
      return -1;
...
int ProgEditWnd::FirstVisibleLine() const
{
   if (!::IsWindow(m_hWnd))
      return -1;
...
int ProgEditWnd::LastVisibleLine() const
{
   if (!::IsWindow(m_hWnd))
      return -1;
...
int ProgEditWnd::VisibleLines(bool bTotal) const
{
   if (!::IsWindow(m_hWnd))
      return 0;
...
CPoint ProgEditWnd::_DrawLine(CDC *pDC, const CString &csLine, const CScriptText::lineVector &colors, int lineidx, int y)
{
...
CRect ProgEditWnd::GetLineRect(int iLine) const
{
   if (!::IsWindow(m_hWnd))
   {
      ASSERT(0);
...
CRect ProgEditWnd::GetLineRect(int iLine1, int iLine2) const
{
   if (!::IsWindow(m_hWnd))
   {
      ASSERT(0);
...
/////////////////////////////////////////////////////////////////////////////
// ProgEditWnd message handlers

void ProgEditWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
   try
   {
...
void ProgEditWnd::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
   try
   {
...
void ProgEditWnd::InsertChar(int pChar, bool pRedraw)
{
...
void ProgEditWnd::InsertString(CString &csString, bool pRedraw)
{
...
void ProgEditWnd::DeletePrevChar()
{
...
//delete the following character
void ProgEditWnd::DeleteNextChar()
{
   if (CursorPosY() >= GetTextSize())
   {
      // nothing to do
...
void ProgEditWnd::DrawCursor(bool pShow)
{
   if (GetFocus()!=this)
   {
      return;
...
void ProgEditWnd::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void ProgEditWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
   point.x -= MIN_X;
...
void ProgEditWnd::OnLButtonUp(UINT nFlags, CPoint point) 
{
   point.x -= MIN_X;
   m_bLBDown = false;
...
bool ProgEditWnd::IsSelected(int pX,int pY)
{
   if (!Selected())
   {
      return(false);
...
void ProgEditWnd::RemoveSelected()
{
...
void ProgEditWnd::ScrollToLine(int iLine)
{
   if (!::IsWindow(m_hWnd))
      return;
...
void ProgEditWnd::SoftScrollToLine(int iLine)
{
...
void ProgEditWnd::ScrollToCursor()
{
...
void ProgEditWnd::UpdateAnchor(bool pTestShift)
{
   if (pTestShift)
   {
      int shift = GetKeyState(VK_SHIFT);
...
BOOL ProgEditWnd::OnEraseBkgnd(CDC* pDC) 
{
   /*
   CRect client;
...
void ProgEditWnd::UpdateScrollSizes()
{
...
void ProgEditWnd::OnSize(UINT nType, int cx, int cy)
{
   UpdateScrollSizes();
}
...
BOOL ProgEditWnd::OnMouseWheel(UINT fFlags, short zDelta, CPoint point)
{
...
void ProgEditWnd::OnVScroll( UINT nSBCode, UINT nPos, CScrollBar* pScrollBar )
{
   DrawCursor(false);
   CRect client;
   GetClientRect(client);
...
void ProgEditWnd::OnHScroll( UINT nSBCode, UINT nPos, CScrollBar* pScrollBar )
{
   DrawCursor(false);
...
void ProgEditWnd::ShiftDisplay(int dx, int dy)
{
   CRect cli;
   GetClientRect(cli);
...
void ProgEditWnd::ReplaceSelText(const char *pText)
{
   StartAction();
...
void ProgEditWnd::_ReplaceSelText(const char *pText)
{
   try
   {
      if (Selected()) 
...
int ProgEditWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
BOOL ProgEditWnd::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
   CRect client;
   GetClientRect(client);
   CPoint cursorpt;
...
void ProgEditWnd::OnShowWindow(BOOL bShow, UINT nStatus)
{
   if (bShow)
   {
      CClientDC dc(this);
...
CString ProgEditWnd::MakeTab(int iCursorXPos) const
{
   CString tab = " ";
   iCursorXPos++;
...
void ProgEditWnd::NotifyParent(UINT msg)
{
   if (!::IsWindow(m_hWnd))
   {
      return;
...
void ProgEditWnd::InsertTextLine(const CString &line, int i, bool bUpdate, bool bNoUndo)  
{
   if (!bNoUndo)
   {
      CUndoAction act;
...
void ProgEditWnd::SetTextLine(const CString &line, int i, bool bNoUndo, bool pUpdate)
{
   if (!bNoUndo)
   {
      CUndoAction act;
...
void ProgEditWnd::RemoveTextLine(int i, bool bNoUndo, bool pUpdate)                       
{
   if (i >= GetTextSize())
      return;
...
void ProgEditWnd::RemoveAllText()                             
{
   CUndoAction act;
...
void ProgEditWnd::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	SetFocus();
...
void ProgEditWnd::OnUpdateCopy(CCmdUI* pCmdUI) 
{
...
void ProgEditWnd::OnUpdateCut(CCmdUI* pCmdUI) 
{
   pCmdUI->Enable(Selected());
}
...
void ProgEditWnd::OnUpdatePaste(CCmdUI* pCmdUI) 
{
	if (OpenClipboard())
	{
...
void ProgEditWnd::OnUpdateSelectall(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(true);
}
...
void ProgEditWnd::OnUpdateUndo(CCmdUI* pCmdUI) 
{
...
void ProgEditWnd::OnUpdateRedo(CCmdUI* pCmdUI) 
{
...
void ProgEditWnd::ClearUndoHistory()
{
   m_currentUndoFrame.Clear();
   ClearUndoStack(m_redoStack);
   ClearUndoStack(m_undoStack);
...
void ProgEditWnd::StartUndoFrame()
{
   m_currentUndoFrame.Clear();
   ClearUndoStack(m_redoStack);
}
...
void ProgEditWnd::DoUndoRedo(undostack &inStack, undostack& outStack)
{
   CUndoFrame inFrame;
   if (inStack.empty())
   {
...
void ProgEditWnd::ClearUndoStack(undostack &inStack)
{
   while (!inStack.empty())
   {
      inStack.pop();
...
void ProgEditWnd::ClipAnchorPosition(int &x, int &y)
{
   CPoint t(x, y);
...
void ProgEditWnd::ClipAnchorPosition(CPoint &p)
{
   p.y = max(0, p.y);
...
void ProgEditWnd::ClipCursorPosition(int &x, int &y)
{
   CPoint t(x, y);
...
void ProgEditWnd::ClipCursorPosition(CPoint &p)
{
   p.y = max(0, p.y);
   p.y = min(GetTextSize() - 1, p.y);
...
void ProgEditWnd::SetSelection(int x1, int y1, int x2, int y2)
{
...
void ProgEditWnd::SetCursorPos(CPoint pos)
{
...
void ProgEditWnd::SetAnchorPos(CPoint pos)
{
...
void ProgEditWnd::OnKillFocus(CWnd* pNewWnd) 
{
...
void ProgEditWnd::OnSetFocus( CWnd* pOldWnd )
{
   DrawCursor(true);
...
BOOL ProgEditWnd::PreTranslateMessage(MSG* pMsg) 
{
   if (m_bUseKBAccel)
   {
...
void ProgEditWnd::SetCharXSpacing(int i)
{
   i = max(0,i);
   m_iCharDX += i;
}
...
void ProgEditWnd::SetCharYSpacing(int i)
{
   i = max(0,i);
   m_iCharDY += i;
}
...
void ProgEditWnd::SetColorizer(CColorizer *pColorizer)
{
...
void ProgEditWnd::SetBackgroundColor(COLORREF clr)
{
...
void ProgEditWnd::SetHighlightColor(COLORREF clr)
{
   m_hilightColor = clr;
   Invalidate(true);
}
...
void ProgEditWnd::SetHighlightTextColor(COLORREF clr)
{
...
void ProgEditWnd::SetLine(const char *pText, int iLine)
{
   DrawCursor(false);
...
void ProgEditWnd::OnLButtonDblClk( UINT nFlags, CPoint point)
{
